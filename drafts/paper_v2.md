...
**Method-effect audit at matched backbone tier (R2-CE vs EDL).** Adding W3-S (matched-protocol R2 baseline at the W3-M tier) lets us separate the *method* effect from the *backbone+context* effect for the first time. At the 0.21 M PointNet-Vanilla tier the CE-vs-EDL mIoU effect is +0.6 pp for CE (W3-G > W3-F by 0.62 pp), confounded by EDL's higher ECE benefit (4× win). At the 0.49 M PointNet2Lite_v2 + warm-restart + 600 fr/seq tier, the CE-vs-EDL mIoU effect is **+0.78 pp for EDL** (W3-M 23.32 % vs W3-S 22.54 %); the ECE effect *reverses* — CE achieves 0.089 vs EDL 0.125. The §III.B comparative thesis as originally stated ("EDL trades small mIoU for big ECE win") therefore holds at the small backbone (where the EDL KL regulariser prevents catastrophic overconfidence) but does not hold at the larger backbone (where CE is already well-calibrated). This is a *publishable* nuance the v2-draft missed; round-1 R1 W1 demanded it and W3-S delivers the evidence.
